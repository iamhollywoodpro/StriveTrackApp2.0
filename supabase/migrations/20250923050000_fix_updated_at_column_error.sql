-- Fix the "updated_at" column error in comprehensive app migration
-- Error: column "updated_at" does not exist
-- Solution: Add updated_at column to goals table and fix all references

-- First, add the missing updated_at column to goals table
...
DROP TRIGGER IF EXISTS update_goals_updated_at_trigger ON public.goals;
CREATE TRIGGER update_goals_updated_at_trigger
    BEFORE UPDATE ON public.goals
    FOR EACH ROW
    EXECUTE FUNCTION public.update_goals_updated_at();

-- Also add updated_at to habits table if it doesn't exist (for consistency)
ALTER TABLE public.habits 
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;

-- Add trigger to automatically update updated_at column for habits
...
-- Create trigger for habits updated_at
DROP TRIGGER IF EXISTS update_habits_updated_at_trigger ON public.habits;
CREATE TRIGGER update_habits_updated_at_trigger
    BEFORE UPDATE ON public.habits
    FOR EACH ROW
    EXECUTE FUNCTION public.update_habits_updated_at();

-- Now recreate the corrected functions from the previous migration
...
-- Create function to update goal progress (NOW WITH CORRECT COLUMN REFERENCE)
...
BEGIN
    ...
    -- Update the goal progress (updated_at will be automatically set by trigger)
    UPDATE public.goals 
...
-- Add indexes for better performance (NOW WITH CORRECT COLUMN REFERENCES)
...
-- Update existing goals to have current timestamp for updated_at
UPDATE public.goals 
SET updated_at = created_at 
WHERE updated_at IS NULL;

-- Update existing habits to have current timestamp for updated_at  
